...
def test_template_flows_through_queue(tmp_path) -> None:
    ...
    assert stats == {"sent": 52, "skipped": 0}
    assert len(queue) == 52

    output = tmp_path / "cases.json"
    records = run_consumer(queue, output)
    assert len(queue) == 0
    assert [r.case_id for r in records] == [f"template#{i:05d}" for i in range(52)]
    assert len(json.loads(output.read_text(encoding="utf-8"))) == 52
